...
server.xml的配置基本默认，只需要修改一个user标签即可，用来指定MyCat操作的用户名密码以及逻辑数据库（MyCat的使用和MySQL基本一致，只是对SQL语句的语法要求更为严格）
...
在schema.xml中指定了负载均衡规则为myrule，现在需要在rule.xml中选择需要的负载均衡算法，选择完负载均衡算法后需要在最后选择的算法中指定节点个数。
...
在MyCat的安装目录的bin目录下执行`./mycat start`即可启动MyCat，通过`./mycat status`查看启动状态。然后通过`mysql -uroot -P8066 -h192.168.2.129 -p`命令即可连接到MyCat，此时MyCat中的数据库和数据表都为逻辑数据库和逻辑数据表。真正的数据库和数据表是在schema.xml中配置的，至此通过操作MyCat即可达到分库分表和读写分离的效果。
